...
class CryptLib {
    ...
    private fun encryptDecrypt(
        ...
        var len =
            _encryptionKey.toByteArray(charset("UTF-8")).size // length of the key	provided
...
//_keyGen.init(128);
        val keySpec =
            SecretKeySpec(_key, "AES") // Create a new SecretKeySpec
        // for the
// specified key
// data and
// algorithm
// name.
        val ivSpec =
            IvParameterSpec(_iv) // Create a new
        // IvParameterSpec
// instance with the
// bytes from the
// specified buffer
...
// vector.
// encryption
        if (_mode == EncryptMode.ENCRYPT) { // Potentially insecure random numbers on Android 4.3 and older.
// Read
...
// for more info.
            ...
            val results =
                _cx.doFinal(_inputText.toByteArray(charset("UTF-8"))) // Finish
            // multi-part
// transformation
// (encryption)
            _out = Base64.encodeToString(
                results,
                Base64.DEFAULT
            ) // ciphertext
            // output
        ...
        if (_mode == EncryptMode.DECRYPT) {
            _cx.init(
                ...
                keySpec,
                ivSpec
            ) // Initialize this ipher instance
...
// (decryption)
    ...
    init { // initialize the cipher with transformation AES/CBC/PKCS5Padding
